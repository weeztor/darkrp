			...
			WepCat:SetLabel("Weapons")
				...
				WepPanel:EnableVerticalScrollbar(true)
					...
					for k,v in pairs(CustomShipments) do
						if v.seperate and ((GetGlobalInt("restrictbuypistol") == 1 and LocalPlayer():Team() == TEAM_GUN) or GetGlobalInt("restrictbuypistol") == 0 or (GetGlobalInt("restrictbuypistol") == 1 and (not v.allowed[1] or table.HasValue(v.allowed, LocalPlayer():Team())))) then
	...
	function HUDTABpanel:Update()
		...
		jobs1cat:SetLabel("Jobs/wallet foreground")
			...
			resetjobs1.DoClick = function()
				...
				LocalPlayer():ConCommand("Job2b 0")
		...
		jobs2cat:SetLabel("Jobs/wallet background")
			...
			resetjobs2.DoClick = function()
				...
				LocalPlayer():ConCommand("Job1b 150")
				LocalPlayer():ConCommand("Job1a 200")
			end
