...
function FAdmin.Log(text)
	...
	file.Write(LogFile, file.Read(LogFile).."\n"..os.date().. "\t"..text)
end
...
hook.Add("CanTool", "FAdmin_Log", function(ply, tr, toolclass) FAdmin.Log(ply:Nick().." ("..ply:SteamID()..") Attempted to use "..toolclass)  end)
...
hook.Add("PlayerSay", "FAdmin_Log", function(ply, text, teamonly, dead) FAdmin.Log(ply:Nick().." ("..ply:SteamID()..") [".. ((dead and "dead, ") or "")..((teamonly and "team only") or "all") .."] "..text) end)
...
hook.Add("EntityRemoved", "FAdmin_Log", function(ent) if ValidEntity(ent) and ent:GetClass() == "prop_physics" then FAdmin.Log(ent:GetClass().. "(" .. (ent:GetModel() or "<no model>") .. ") Got removed") end end)
